...
export class NotebookDialog extends Dialog {
    ...
    private async generateDialogHtml(notebookInfo: NotebookInfo): Promise<string> {
    ...
    async registerDialog(buttons?: ButtonSpec[]) {
        ...
        await joplin.views.dialogs.addScript(this._dialog, './ui/styles/notebookDialogStyle.css');
        await joplin.views.dialogs.addScript(this._dialog, './ui/controllers/notebookDialogController.js');
        if (buttons) {
            await joplin.views.dialogs.setButtons(this._dialog, buttons);
        }
    }
