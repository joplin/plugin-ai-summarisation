import { LexRankHandler } from '../../src/utils/handlers/lex-rank/LexRankHandler';

describe('LexRankHandler', () => {
  const handler = new LexRankHandler();

  it('should correctly summarize a note', () => {
    const note = `
    ...
    `;    
    const summary = handler.predict(note);
